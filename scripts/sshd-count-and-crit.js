...
function transform(acc, lines) {
  ...
  let out = lines
    ...
    .map(line => {
      return Object.assign({}, line, { syslog: parse(line.r) });
    })
    .filter(line => {
      return line.syslog.facility === "auth";
    })
    .filter(line => {
      a[line.syslog.severity] = (a[line.syslog.severity] || 0) + 1;
      return line.syslog.severity === "crit";
    })
    .map(line => {
      return JSON.stringify(
        Object.assign({}, line, {
          r: line.syslog.message + JSON.stringify(line.syslog)
        })
      );
    });
